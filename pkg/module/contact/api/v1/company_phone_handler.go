...
func (api *apiV1) GetCompanyPhonesHandlerV1(w http.ResponseWriter, r *http.Request) {
	...
	for _, item := range result {
		response.Items = append(response.Items, CompanyPhoneToListItemViewModel(item, language, api.service.GetLanguageProvider().DefaultLanguage(ctx)))
	}
...
func (api *apiV1) GetCompanyPhoneByIdHandlerV1(w http.ResponseWriter, r *http.Request) {
	...
	response := CompanyPhoneToViewModel(result, language, api.service.GetLanguageProvider().DefaultLanguage(ctx))
	rest.WriteResult(w, response)
}
